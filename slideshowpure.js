    ...
    if (
      ...
      if (!STATE.slideshow.hasInitialized) {
        initJellyfinData(async () => {
          ...
          // NOTE: The YouTube iframe API is loaded lazily per-slide inside
          // createSlideForItemId(). Do NOT await it here — doing so blocks the
          // entire slideshow (and first paint) on a third-party youtube.com
          // script load.
          slidesInit();
        });
      } else {
...
const ApiUtils = {
  ...
  async fetchItemIdsFromServer() {
    try {
      ...
      // Request every field a slide actually renders, so the bulk call doubles
      // as the per-item detail fetch. Without this, each slide triggers a
      // separate /Items/{id} request (an N+1 pattern that gates first paint on
      // a second sequential round-trip).
      const fields = [
        "Overview",
        "Genres",
        "CommunityRating",
      ...
      const response = await fetch(
        `${STATE.jellyfinData.serverAddress}/Items?IncludeItemTypes=Movie,Series&Recursive=true&hasOverview=true&imageTypes=Logo,Backdrop&sortBy=Random&isPlayed=False&enableUserData=true&Limit=${CONFIG.maxItems}&fields=${fields}`,
        {
          headers: this.getAuthHeaders(),
        },
      );
      ...
      // Pre-populate the detail cache so createSlideForItemId() →
      // fetchItemDetails() serves from memory instead of issuing a per-slide
      // network request.
      usableItems.forEach((item) => {
        STATE.slideshow.loadedItems[item.Id] = item;
      });

      return usableItems.map((item) => item.Id);
    } catch (error) {
...
const SlideCreator = {
  ...
  async createSlideForItemId(itemId) {
    try {
      ...
      if (videoId) {
        // Load the YouTube API and SponsorBlock skip data concurrently and OFF
        // the critical path. Neither is awaited here, so the slide is returned
        // (and made visible by updateCurrentSlide) immediately instead of
        // waiting on an external sponsor.ajay.app round-trip.
        Promise.all([
          loadYouTubeAPI(),
          ApiUtils.getSkipSegments(videoId),
        ]).then(([YT, startTime]) => {
